...
function __pulse_jwt_sign_private_key(encoded: Uint8Array, policy: __PulseFastlyValue): Uint8Array | null {
  ...
  const allowed = ['kty', 'crv', 'x', 'y', 'd', 'alg', 'use', 'key_ops', 'kid', 'ext']
  for (let i = 0; i < jwk.keys.length; i++) if (allowed.indexOf(jwk.keys[i]) < 0) return null
  if (__pulse_jwt_string_member(jwk, 'kty') != 'EC' || __pulse_jwt_string_member(jwk, 'crv') != 'P-256') return null
  if (__pulse_jwt_has_field(jwk, 'alg') && __pulse_jwt_string_member(jwk, 'alg') != 'ES256') return null
...
export function pulse_jwt_fastly_sign(effectIndex: i32, outerHandle: i32): i32 {
  ...
  if ((algorithm != 'HS256' && algorithm != 'ES256') || !isFinite(lifetime)
  ...
  const tag = new Uint8Array(algorithm == 'ES256' ? 64 : 32)
