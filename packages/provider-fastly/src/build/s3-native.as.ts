...
class __PulseS3Binding {
  endpoint: string = ''; bucket: string = ''; region: string = ''; backend: string = '';
...
function __s3_fail(index: i32, reason: string): void { __pulse_fastly_ready_effect(index, __s3_result('failed', reason)) }
...
function __pulse_fastly_s3_begin(index: i32, outer: __PulseFastlyValue): void {
  ...
  const method = __pulse_fastly_effect_kind(index) == 12 ? 'HEAD' : 'GET', path = '/' + binding.bucket + '/' + encoded;
  const now = __s3_clock(0);
  if (now < 0 || !__pulse_s3_date(now / 1000000000).length) { __s3_fail(index, 'configuration'); return; }
  const signature = __pulse_s3_sign(method, path, binding.endpoint.slice(8), binding.region, id, secret, token, now / 1000000000);
...
function __s3_read_response(index: i32, response: i32, body: i32, status: i32): i32 {
  if (status == 404) return __s3_result('not-found');
  if (status != 200) return __s3_result('failed', __pulse_s3_status_reason(status), status);
  const buffer = new Uint8Array(16384), end = new StaticArray<i64>(1), size = new StaticArray<i32>(1);
  if (__s3_header_names(response, buffer.dataStart, buffer.length, 0, changetype<usize>(end), changetype<usize>(size)) != 0 || end[0] >= 0) return __s3_result('failed', 'protocol', status);
  const names = __s3_list(buffer, size[0]); if (names === null) return __s3_result('failed', 'protocol', status);
  let total = 0, length: f64 = -1; const seen = new Set<string>();
  const result = __s3_result('found');
  for (let i = 0; i < names.length; i++) {
    const rawName = names[i], name = rawName.toLowerCase(), bytes = __pulse_s3_bytes(rawName);
    if (__s3_header_values(response, bytes.dataStart, bytes.length, buffer.dataStart, buffer.length, 0, changetype<usize>(end), changetype<usize>(size)) != 0 || end[0] >= 0) return __s3_result('failed', 'protocol', status);
    const values = __s3_list(buffer, size[0]); if (values === null) return __s3_result('failed', 'protocol', status);
    const selected = name == 'content-length' || name == 'content-type' || name == 'etag' || name == 'content-encoding';
    if (selected && (seen.has(name) || values.length != 1)) return __s3_result('failed', 'protocol', status);
    seen.add(name);
    for (let v = 0; v < values.length; v++) {
      const value = values[v], count = __pulse_s3_bytes(value).length;
      total += bytes.length + count + 4; if (total > 16384) return __s3_result('failed', 'protocol', status);
      if (!selected) continue;
      if (count > 1024 || !__pulse_s3_scalar(value, true)) return __s3_result('failed', 'protocol', status);
      if (name == 'content-encoding' && value.toLowerCase() != 'identity') return __s3_result('failed', 'protocol', status);
      if (name == 'content-length') {
        if (!value.length || (value.length > 1 && value.charCodeAt(0) == 48)) return __s3_result('failed', 'protocol', status);
        length = 0;
        for (let c = 0; c < value.length; c++) { const digit = value.charCodeAt(c) - 48; if (digit < 0 || digit > 9) return __s3_result('failed', 'protocol', status); length = length * 10 + digit; if (length > 9007199254740991) return __s3_result('failed', 'protocol', status); }
      }
      if (name == 'etag') { if (!value.length) return __s3_result('failed', 'protocol', status); __s3_set(result, 'etag', __pulse_fastly_string_value(value)); }
      if (name == 'content-type') __s3_set(result, 'contentType', __pulse_fastly_string_value(value));
    }
  }
  if (__s3_remaining(index) == 0) return __s3_result('failed', 'timeout', status);
  const head = __pulse_fastly_effect_kind(index) == 12, binding = __pulse_fastly_s3_binding(index);
  if (head && length < 0) return __s3_result('failed', 'protocol', status);
  if (!head) {
    if (length > binding.max) return __s3_result('failed', 'too-large', status);
    const data = new Uint8Array(binding.max + 1); let count = 0;
    while (true) {
      const ready = __s3_wait_ready(index, body);
      if (ready <= 0) return __s3_result('failed', ready == 0 ? 'timeout' : 'transport', status);
      if (fastly_http_body_read(body, data.dataStart + count, data.length - count, changetype<usize>(size)) != 0) return __s3_result('failed', 'protocol', status);
      if (size[0] < 0 || size[0] > data.length - count) return __s3_result('failed', 'protocol', status);
      if (size[0] == 0) break;
      count += size[0]; if (count > binding.max) return __s3_result('failed', 'too-large', status);
    }
    if (length >= 0 && length != count) return __s3_result('failed', 'protocol', status);
    length = f64(count); const raw = data.subarray(0, count), digest = __pulse_s3_hex(__pulse_s3_sha(raw));
    if (!__pulse_s3_utf8_valid(raw)) return __s3_result('failed', 'invalid-utf8', status);
...
function __pulse_fastly_s3_wait(index: i32): i32 {
  ...
  while (true) {
    ...
    if (ready <= 0) return __s3_result('failed', ready == 0 ? 'timeout' : 'transport');
    if (__s3_poll(pending, changetype<usize>(detail), changetype<usize>(done), changetype<usize>(response), changetype<usize>(body)) != 0) return __s3_result('failed', 'transport');
    if (done[0] != 0) break;
  }
  let result: i32;
  if (fastly_http_resp_status_get(response[0], changetype<usize>(code)) != 0) result = __s3_result('failed', 'protocol');
  else result = __s3_remaining(index) == 0 ? __s3_result('failed', 'timeout', code[0]) : __s3_read_response(index, response[0], body[0], code[0]);
