...
const CTX_SURFACE_IDS = Object.freeze([
  ...
  'ctx.state.get','ctx.state.set','ctx.encodeJson','ctx.json','ctx.text','ctx.response','ctx.req.text','ctx.req.json.schema',
...
function defaultCtxContract() {
  return Object.freeze({
    ...
    planes: Object.freeze({
      sharedAuthoring: Object.freeze(['state', 'log', 'fetch', 'parallel', 'config', 'secret', 'kv', 'emit', 'encodeJson']),
