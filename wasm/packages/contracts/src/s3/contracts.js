...
const S3_OPERATIONS = Object.freeze(Object.fromEntries(['head', 'getText'].map((name) => [name, Object.freeze({
  kind: `s3.${name}`, capability: `s3.${name}`, result: name === 'head' ? 's3-head-result' : 's3-get-text-result'
})])));
const S3_PROVIDER_REQUIREMENTS = Object.freeze(['s3.head', 's3.getText', 'secret.get', 'time.wall-clock']);
