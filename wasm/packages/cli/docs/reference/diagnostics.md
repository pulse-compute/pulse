...
Pulse 1.0.0-beta.5 treats the 126 codes on this page as the stable public CLI diagnostic catalog. Code, category, summary, remediation intent, exit class, and anchor are generated from the same source used by the CLI. Human wording may improve compatibly.

Every catalogued failure links to a versioned installed artifact under:

`https://pulsecompute.io/v1.0.0-beta.5/reference/diagnostics/`
...
```json
{
  ...
  "error": {
    ...
    "scope": "public",
    "docs": "https://pulsecompute.io/v1.0.0-beta.5/reference/diagnostics/#pulse-build-out-unsafe"
  }
}
```
...
|---:|---|
| `400` | `PULSE_SCHEMA_JSON_MALFORMED`, `PULSE_BODY_DECODE`, `PULSE_REQUEST_BODY_INVALID_UTF8`, `PULSE_SCHEMA_DECODE`, `PULSE_OPAQUE_BODY_INSPECTION`, `PULSE_FETCH_URL_INVALID` |
...
| `504` | `PULSE_REQUEST_DEADLINE_EXCEEDED`, `PULSE_REQUEST_CLOCK_INVALID`, `PULSE_FETCH_TIMEOUT`, `PULSE_CONTINUATION_EXPIRED`, `PULSE_FASTLY_REQUEST_TIMEOUT` |
  ...
  "cases": [
    {
      ...
      "expectedError": {
        ...
        "category": "request",
        "docs": "https://pulsecompute.io/v1.0.0-beta.5/reference/diagnostics/#pulse-schema-decode"
      }
    }
  ]
}
...
<a id="pulse-request-duration-invalid"></a>
### `PULSE_REQUEST_DURATION_INVALID` — Request Duration Invalid

The configured request duration is outside its supported range.
...
1. Set provider maxDurationMs to an integer from 1 through 30000, or omit it.

<a id="pulse-request-duration-unsupported"></a>
### `PULSE_REQUEST_DURATION_UNSUPPORTED` — Request Duration Unsupported

The selected target cannot enforce the configured request deadline.
...
1. Select Fastly Native for a provider-owned request deadline.

<a id="pulse-request-duration-artifact-mismatch"></a>
### `PULSE_REQUEST_DURATION_ARTIFACT_MISMATCH` — Request Duration Artifact Mismatch

The Fastly artifact deadline differs from the selected profile.
...
1. Rebuild the Fastly Native artifact using the selected profile.
...
<a id="pulse-native-text-unsupported"></a>
### `PULSE_NATIVE_TEXT_UNSUPPORTED` — Native Text Unsupported

Diagnostic WebAssembly text is available only for Native compilation.
...
1. Select a Native build profile or use `pulse compile --emit-wat`.
2. Remove `--emit-wat` for JavaScript builds.
...
<a id="pulse-canonical-pure-loop-unsupported"></a>
### `PULSE_CANONICAL_PURE_LOOP_UNSUPPORTED` — Canonical Pure Loop Unsupported

The loop does not satisfy the bounded pure-value control-flow contract.
...
1. Use a let counter starting at zero, a literal cap first, and an increment of one.
2. Keep the body within bounded pure value operations and move context operations outside the loop.
3. Respect the documented per-loop and nested iteration limits.

<a id="pulse-canonical-read-loop-unsupported"></a>
### `PULSE_CANONICAL_READ_LOOP_UNSUPPORTED` — Canonical Read Loop Unsupported

The loop does not satisfy the bounded sequential read control-flow contract.
...
1. Use a let counter starting at zero, a literal cap from 0 through 64 first, and an increment of one.
2. Use sequential s3.getText, kv.getVersioned or crypto.digestText calls directly bound to one local or discarded.
3. Keep nested loops pure; avoid parallel effects, writes, callbacks, counter mutation and labelled transfers.
...
<a id="pulse-schema-json-limits-invalid"></a>
### `PULSE_SCHEMA_JSON_LIMITS_INVALID` — Schema JSON Limits Invalid

A schema JSON admission policy contains an unknown limit or an invalid bounded integer.
...
1. Use only documented JSON limit names with positive safe-integer values.
2. Set maxDepth to at most 128 for a Native target.

<a id="pulse-schema-json-limit-literal-required"></a>
### `PULSE_SCHEMA_JSON_LIMIT_LITERAL_REQUIRED` — Schema JSON Limit Literal Required

A schema JSON limit cannot be resolved from a static numeric literal.
...
1. Write JSON limits as numeric literals inside schema<T>({ json: { ... } }).
2. Remove computed values, spreads, and accessors from schema options.

<a id="pulse-schema-json-depth-unsupported"></a>
### `PULSE_SCHEMA_JSON_DEPTH_UNSUPPORTED` — Schema JSON Depth Unsupported

A schema depth limit exceeds the Native parser capacity of 128 container levels.
...
1. Set the schema maxDepth limit to at most 128 for Native compilation.
2. Pulse does not clamp limits or select another target automatically.
...
<a id="pulse-request-body-invalid-utf8"></a>
### `PULSE_REQUEST_BODY_INVALID_UTF8` — Request Body Invalid Utf8

Request bytes are not well-formed UTF-8; no repaired text is exposed to the application.
...
1. Send well-formed UTF-8 request bytes; Pulse does not repair malformed input.
...
<a id="pulse-request-deadline-exceeded"></a>
### `PULSE_REQUEST_DEADLINE_EXCEEDED` — Request Deadline Exceeded

The provider-owned total request budget expired.
...
1. Split the work into bounded invocations or deliberately adjust the provider maxDurationMs profile. A timeout does not prove rollback of dispatched writes.

<a id="pulse-request-clock-invalid"></a>
### `PULSE_REQUEST_CLOCK_INVALID` — Request Clock Invalid

The provider monotonic request clock failed or regressed.
...
1. Restore the provider monotonic clock before retrying. Reconcile any dispatched writes.
