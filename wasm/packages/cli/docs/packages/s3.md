...
`maxTextBytes: 2097152` and `schemas.maxBytes: 2097152`. When a request carries
text inside JSON, control escapes and wrapper overhead count against the
separate structured-body ceiling; the encoded request must fit that bound.
