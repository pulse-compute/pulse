...
const DIAGNOSTIC_DEFINITIONS = Object.freeze({
  ...
  PULSE_NATIVE_TEXT_UNSUPPORTED: entry('capability', { exitCode: 2, remediation: ['Select a Native build profile or use `pulse compile --emit-wat`.', 'Remove `--emit-wat` for JavaScript builds.'] }),
  ...
  PULSE_CANONICAL_READ_LOOP_UNSUPPORTED: entry('compile', { exitCode: 3, remediation: ['Use a let counter starting at zero, a literal cap from 0 through 64 first, and an increment of one.', 'Use sequential s3.getText, kv.getVersioned or crypto.digestText calls directly bound to one local or discarded.', 'Keep nested loops pure; avoid parallel effects, writes, callbacks, counter mutation and labelled transfers.'] }),
  ...
  PULSE_SCHEMA_JSON_LIMITS_INVALID: entry('schema', { exitCode: 3, remediation: ['Use only documented JSON limit names with positive safe-integer values.', 'Set maxDepth to at most 128 for a Native target.'] }),
  PULSE_SCHEMA_JSON_LIMIT_LITERAL_REQUIRED: entry('schema', { exitCode: 3, remediation: ['Write JSON limits as numeric literals inside schema<T>({ json: { ... } }).', 'Remove computed values, spreads, and accessors from schema options.'] }),
  PULSE_SCHEMA_JSON_DEPTH_UNSUPPORTED: entry('schema', { exitCode: 3, remediation: ['Set the schema maxDepth limit to at most 128 for Native compilation.', 'Pulse does not clamp limits or select another target automatically.'] }),
...
const DIAGNOSTIC_SUMMARIES = Object.freeze({
  ...
  PULSE_NATIVE_TEXT_UNSUPPORTED: 'Diagnostic WebAssembly text is available only for Native compilation.',
  ...
  PULSE_CANONICAL_READ_LOOP_UNSUPPORTED: 'The loop does not satisfy the bounded sequential read control-flow contract.',
  ...
  PULSE_SCHEMA_JSON_LIMITS_INVALID: 'A schema JSON admission policy contains an unknown limit or an invalid bounded integer.',
  PULSE_SCHEMA_JSON_LIMIT_LITERAL_REQUIRED: 'A schema JSON limit cannot be resolved from a static numeric literal.',
  PULSE_SCHEMA_JSON_DEPTH_UNSUPPORTED: 'A schema depth limit exceeds the Native parser capacity of 128 container levels.',
