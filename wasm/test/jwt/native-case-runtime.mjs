...
export const NATIVE_CASE_RUNTIME_VERSION =
...
function validNativeRequest(request) {
...
function immutableResultObservation(result, schemaValue) {
  ...
  return Object.freeze({
    ...
    claimKeys: ordinaryObject(claims)
      ? Object.freeze(Object.keys(claims).sort())
      : Object.freeze([]),
...
export async function executeNativeSemanticCase(input, authority, guestVerifier) {
  ...
  const selectedCrypto = Object.freeze({
    mac: Object.freeze({
      async verify(request) {
        ...
        const keyBytes = dataValue(dataValue(request, 'key'), 'bytes');
        const data = dataValue(request, 'data');
        const tag = dataValue(request, 'tag');
        observed.cryptoKeyBytes = byteLength(keyBytes);
        observed.cryptoDataBytes = byteLength(data);
        observed.cryptoDataSha256 = data instanceof Uint8Array
          ? nodeCrypto.createHash('sha256').update(data).digest('hex')
          : null;
        observed.cryptoTagBytes = byteLength(tag);
        ...
        if (validNativeRequest(request)) observed.cryptoPrimitiveCalls += 1;
        const result = await guestVerifier.mac.verify(request);
  ...
  try {
    ...
    return Object.freeze({
      version: NATIVE_CASE_RUNTIME_VERSION,
      status: 'success',
