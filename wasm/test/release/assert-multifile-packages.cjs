...
async function main() {
  ...
  assert.ok(rootArgument && releaseArgument, 'Usage: assert-multifile-packages.cjs <isolated-install> <release-pack>');
  ...
  if (!execute) {
    ...
    verifyClosure(packedRoot, releaseDir, manifest);
    console.log(JSON.stringify({ ...result, schemaVersion: 'pulse.multifile-packed.v1',
      packageCount: manifest.packageCount, packages: manifest.packages.map(({ name, version, sha256 }) => ({ name, version, sha256 })),
      installedBytesUnchanged: true, incompleteClosureRejected: true, alteredCompilerRejected: true, regeneratedArtifactsMatch: true }));
    return;
  }

  const result = await require('../crypto/assert-multifile-source-identity.cjs').main({ packedRoot, quiet: true });
