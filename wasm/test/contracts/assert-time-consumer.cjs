...
const root = installed ? path.resolve(installed) : fs.mkdtempSync(path.join(process.env.PULSEWASM_TEST_TMP_ROOT || os.tmpdir(), 'time-consumer-'));
...
export default defineConfig((_scope) => ({
  pulse: { entry: 'src/index.ts', tests: 'tests/pulse.harness.ts', strict: true },
  ...
  write('src/index.ts', `import { Pulse, type PulseTimeResult } from '@pulse-compute/pulse';
const app = new Pulse({ auto: true });
app.get('/time', async ctx => {
  const sample: PulseTimeResult = await ctx.time.now();
  if (sample.status === 'failed') return ctx.text(sample.reason, { status: 503 });
  if (sample.unixEpochMs <= 0 || sample.iso8601.length !== 24) return ctx.text('invalid', { status: 500 });
  const { first, second } = await ctx.parallel({ first: ctx.time.now(), second: ctx.time.now() });
  if (first.status !== 'ok' || second.status !== 'ok') return ctx.text('failed', { status: 503 });
  return ctx.text('clock ready');
});
export default app;`);
  write('tests/pulse.harness.ts', `export default [{ name: 'public time and grouped samples', request: { method: 'GET', path: '/time' }, expect: { status: 200, text: 'clock ready' } }];`);
  ...
  for (const profile of ['native', 'javascript', 'fastly', 'fastlyjs']) {
    ...
    console.log(`ok - ${profile}: public clock inspect and local consumer`);
  }
  ...
  const response = await executeNodeJavascriptApplication(application, new Request('https://time.test/time'));
  assert.equal(response.status, 200);
  assert.equal(await response.text(), 'clock ready');
  run('build', 'native');
  run('build', 'fastly');
  console.log('ok - built JavaScript execution and Node/Fastly Native builds; no source dependency mutation');
}
